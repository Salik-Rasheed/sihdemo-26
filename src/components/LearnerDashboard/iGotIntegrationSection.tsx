...
export const IGotIntegrationSection: React.FC<IGotIntegrationSectionProps> = ({
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-600 font-medium">
              Intelligent bridge connecting StatSkill AI competency gaps with official Government of India CBC learning resources.
            </p>
          </div>

          <div className="flex items-center space-x-2">
            <span className="px-3 py-1 rounded bg-amber-50 text-amber-900 border border-amber-200 text-xs font-bold">
              Status: API Ready — Prototype Mode
            </span>
            <button
              onClick={onSyncNow}
              disabled={isSyncing}
              className="px-3.5 py-1.5 rounded bg-blue-700 hover:bg-blue-800 text-white font-bold text-xs transition flex items-center space-x-1.5 border border-blue-800"
            >
      ...
      {/* Integration Metrics (Section 16) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="card-panel rounded-xl p-4 bg-white border border-slate-200 shadow-2xs">
          ...
          <span className="text-sm font-black text-amber-800 flex items-center gap-1">
            ...
            API Ready — Prototype Mode
          </span>
          <span className="text-[11px] text-slate-500 font-medium block mt-1">Staging Endpoints</span>
        </div>

        <div className="card-panel rounded-xl p-4 bg-white border border-slate-200 shadow-2xs">
          <span className="text-xs text-slate-500 font-bold block mb-1">Courses Available</span>
          <span className="text-2xl font-black text-blue-700">5,632</span>
        ...
        <div className="card-panel rounded-xl p-4 bg-white border border-slate-200 shadow-2xs">
          ...
          <span className="text-2xl font-black text-emerald-700">1,284</span>
      ...
      <div className="flex space-x-2 border-b border-slate-200 pb-2 text-xs font-bold">
        ...
        >
          iGOT Sync Center & Telemetry Logs ({syncLogs.length})
        </button>
      </div>
      ...
      {activeSubTab === 'catalog' && (
        <div className="space-y-6">
          <div className="card-panel rounded-xl p-4 bg-white border border-slate-200 shadow-2xs flex flex-col sm:flex-row gap-3">
            <div className="relative flex-grow">
              ...
              <input
                ...
                className="w-full bg-slate-50 border border-slate-300 rounded pl-9 pr-3 py-2 text-xs font-medium text-slate-900 outline-none"
              />
            </div>
            <select
              value={selectedCompetency}
              onChange={(e) => setSelectedCompetency(e.target.value)}
              className="bg-slate-50 border border-slate-300 rounded px-3 py-2 text-xs font-bold text-slate-900 outline-none"
            >
            ...
            {filteredCourses.map((c) => (
              ...
              >
                <div>
                  ...
                  <div className="p-3 rounded bg-blue-50/70 border border-blue-200 text-xs text-slate-800 mb-4">
                    <div className="font-bold text-blue-900 mb-1">Why recommended?</div>
                ...
                <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-200">
                  <button
                    ...
                    className="py-2 rounded bg-blue-700 hover:bg-blue-800 text-white font-bold text-xs transition flex items-center justify-center space-x-1 border border-blue-800"
                  >
                  ...
                  <button
                    onClick={() => {
                      if (c.deepLinkUrl) window.open(c.deepLinkUrl, '_blank');
                      else alert("iGOT course link will be configured when authorized integration is connected.");
                    }}
                    className="py-2 rounded bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold text-xs transition border border-slate-300 flex items-center justify-center space-x-1"
                  >
      ...
      {activeSubTab === 'mappings' && (
        <div className="card-panel rounded-xl p-6 bg-white border border-slate-200 shadow-sm overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-100 text-slate-700 font-bold">
                <th className="py-3 px-4">StatSkill / KarmAI Competency</th>
            ...
            <tbody className="divide-y divide-slate-200 font-medium">
              {mappings.map((m) => (
                <tr key={m.id} className="hover:bg-slate-50">
                  ...
                  <td className="py-3 px-4 text-right font-bold text-emerald-700">{m.matchPercentage}%</td>
            ...
            <tbody className="divide-y divide-slate-200 font-medium">
              {syncLogs.map((l) => (
                <tr key={l.id} className="hover:bg-slate-50">
                  ...
                  <td className="py-3 px-4 text-right font-bold text-blue-700">{l.recordsSynced}</td>
                  <td className="py-3 px-4 text-center">
